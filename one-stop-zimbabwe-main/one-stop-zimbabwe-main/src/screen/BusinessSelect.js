...
function BusinessSelect(Props) {
    ...
    let All_Business = Props.All_Business_Nodes_newOld.filter(
    ...
    let renderItem = v => {
        return (
            <View
                ...
                <TouchableOpacity
                    onPress={() => {
                        v.item.user_uid == user.uid  && v.item.type =='business'?
                        
                            setNotify(true)
                            :
                            Props.navigation.navigate(Props.route.params.navigate, { SelectedBusiness: v.item });
                    }}
                    activeOpacity={0.5}
                    style={styles.card}>
